...
void visualize_scatter_plot(t_data_frame*** df, int feat_1, int feat_2) {
	...
	while (i < len) {
		...
		// printw("LEN = %d | HOUSE NUMBER = %d\n", i, color_number);
		attron(COLOR_PAIR(color_number));
		printw("%f | %f", floor(atof(df[i][feat_1]->s)), floor(atof(df[i][feat_2]->s)));
		mvaddch(top_lines + (int)(atof(df[i][feat_1]->s)), (int)(atof(df[i][feat_2]->s)), '+');
		attroff(COLOR_PAIR(color_number));
		i++;
	}
...
int main(int argc, char **argv) {
	...
	if (argc != 4) {
		perror("Wrong amount of arguments. Usage: ./histogram *csv_file* *first_feature_index* *second_feature_index*\n");
		exit(EXIT_FAILURE);
	}
